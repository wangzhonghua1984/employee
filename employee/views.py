...
class DepModelForm(forms.ModelForm):
    ...
    Dep_intro = forms.CharField(widget=forms.Textarea(
        attrs={'rows': 4}), min_length=10, label="Department Introduction")
...
def dep_edit(request, dep_id):
    ...
    models.Departments.objects.filter(id=dep_id).update(
        Dep_name=edit_dep_name, Dep_intro=edit_dep_intro)
    return redirect('/employee/depart/list/')
...
class AppModelForm(forms.ModelForm):
    ...
    App_intro = forms.CharField(widget=forms.Textarea(
        attrs={'rows': 4}), min_length=10, label="Application Introduction")
...
def app_edit(request, app_id):
    ...
    models.Applications.objects.filter(id=app_id).update(
        App_name=edit_app_name, App_intro=edit_app_intro)
    return redirect('/employee/app/list/')
...
class RoleModelForm(forms.ModelForm):
    ...
    Role_intro = forms.CharField(widget=forms.Textarea(
        attrs={'rows': 4}), min_length=10, label="Role Introduction")
...
def role_edit(request, role_id):
    ...
    models.Roles.objects.filter(id=role_id).update(
        Role_name=edit_role_name, Role_intro=edit_role_intro)
    return redirect('/employee/role/list/')
...
class EmpModelForm(forms.ModelForm):
    ...
    Emp_dep = forms.ModelChoiceField(
        queryset=models.Departments.objects.all(), label="Role of Deployment")
    Emp_role = forms.ModelChoiceField(
        queryset=models.Roles.objects.all(), label="Role of Employee")
    Emp_res_apps = forms.ModelMultipleChoiceField(queryset=models.Applications.objects.all(
    ), label="Responsible Applications", widget=Select2MultipleWidget)
...
def emp_edit(request, emp_id):
    ...
    if request.method == "GET":
        emp_form = EmpModelForm(instance=obj)
        return render(request, "emp_edit.html", {
            "emp_form": emp_form
        })

    emp_form = EmpModelForm(data=request.POST, instance= obj)
    if emp_form.is_valid():
        emp_form.save()
        return redirect('/employee/emp/list/')
    else:
